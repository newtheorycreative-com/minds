 ...
 */
if ( !function_exists( 'hub2b_get_template_part' ) ) {

	function hub2b_get_template_part( $slug, $name = null, array $hub2b_params = array() ) {
 ...
 * Print H1 header
*/
if ( !function_exists( 'hub2b_the_h1' ) ) {

	function hub2b_the_h1() {

		$title = hub2b_get_the_h1();
		if ( !empty($title) ) echo '<h1>' . esc_html( $title ) . '</h1>';
	}
}

if ( !function_exists( 'hub2b_get_the_h1' ) ) {

	function hub2b_get_the_h1() {

		if ( is_home() ) {

			$title = esc_html__( 'All Blog Posts', 'hub2b' );
		} 
			else
		if ( is_front_page() ) {

			$title = esc_html__( 'Home', 'hub2b' );
		}
			else
		if ( is_year() ) {

			$title = sprintf( esc_html__( 'Year Archives: %s', 'hub2b' ), get_the_date( 'Y' ) );
		}
			else				
		if ( is_month() ) {

			$title = sprintf( esc_html__( 'Month Archives: %s', 'hub2b' ), get_the_date( 'F Y' ) );
		}
			else
		if ( is_day() ) {

			$title = sprintf( esc_html__( 'Day Archives: %s', 'hub2b' ), get_the_date() );
		}
			else
		if ( is_category() ) {
		...
		if ( is_tag() ) {
			...
			$title = sprintf( esc_html__( 'Tag: %s', 'hub2b' ), single_tag_title( '', false ) );
		}
			else
		if ( is_tax() ) {
		...
		if ( is_search() ) {
			...
			$title = sprintf( esc_html__( 'Search Results: %s', 'hub2b' ), get_search_query() );
		} 
			else				
		if ( is_author() ) {
			...
			$title = sprintf( esc_html__( 'Author: %s', 'hub2b' ), $q->display_name );
		} 
			else
		...
		if ( is_attachment() ) {
			...
			$title = sprintf( esc_html__( 'Attachment: %s', 'hub2b' ), get_the_title() );
		}
			else
		if ( is_404() ) {

			$title = esc_html__( '404 Not Found', 'hub2b' );
		}
			else {
 ...
 */
if ( !function_exists( 'hub2b_add_current_nav_class' ) ) {

	function hub2b_add_current_nav_class( $classes, $item ) {
...
add_action( 'nav_menu_css_class', 'hub2b_add_current_nav_class', 10, 2 );


/**
 * Manual excerpt generation
 */
if ( !function_exists( 'hub2b_excerpt_set' ) ) {

	function hub2b_excerpt_set() {
	...
	add_filter( 'excerpt_length', 'hub2b_excerpt_set', 999 );
}


if ( !function_exists( 'hub2b_excerpt' ) ) {
	
	function hub2b_excerpt( $content, $excerpt = 0 ) {
		...
		if ( ! empty( $post->post_content ) &&
			...
			$content = hub2b_cut_excerpt( $post->post_content , $excerpt );
		}
...
if ( !function_exists( 'hub2b_cut_excerpt' ) ) {
	
	function hub2b_cut_excerpt( $content = '', $excerpt = 0 ) {

		$cut = false;
		$excerpt_more = apply_filters( 'excerpt_more', ' ...' );
		$content = hub2b_get_content( $content );
		...
		$content = hub2b_strip_tags( $texts, $cut );
 ...
 */
if ( !function_exists( 'hub2b_cut_text' ) ) {

	function hub2b_cut_text( $text, $cut = 300, $aft = ' ...' ) {
 ...
 */
if ( !function_exists( 'hub2b_get_content' ) ) {
	
	function hub2b_get_content( $content = '' ) {
...
if ( !function_exists( 'hub2b_strip_tags' ) ) {
	
	function hub2b_strip_tags( $texts = array(), $cut = 0 ) {
 ...
 * @return bool
 */
if ( !function_exists( 'hub2b_is_wc' ) ) {

	function hub2b_is_wc($tag, $attr='') {
 ...
 */
if ( !function_exists('hub2b_is_vc') ) {

    function hub2b_is_vc() {
...
if ( !function_exists( 'v' ) ) {
	...
	function hub2b_plugin_is_active( $plugin_var, $plugin_dir = null ) {
 ...
 */
if ( !function_exists( 'hub2b_admin_css' ) ) {
	
	function hub2b_admin_css() {

		wp_enqueue_style( 'hub2b-admin', get_template_directory_uri() . '/assets/css/admin.css', false, '1.0.0' );
	}
}
add_action( 'admin_enqueue_scripts', 'hub2b_admin_css' );

/**
 * Return inverted contrast value of color
 */
if ( !function_exists( 'hub2b_rgb_contrast' ) ) {
	
	function hub2b_rgb_contrast($r, $g, $b) {
 ...
 */
if ( !function_exists( 'hub2b_color_change' ) ) {
	
	function hub2b_color_change( $hex, $percent ) {
...
function hub2b_adjustBrightness($hex, $steps) {
 ...
 * @return array();
 */
if ( !function_exists( 'hub2b_get_footer_cols_num' ) ) {

	function hub2b_get_footer_cols_num() {
 ...
 */
if ( !function_exists( 'hub2b_get_navbar_layout' ) ) {

	function hub2b_get_navbar_layout( $default = null ) {
		...
		if ( ( !empty( $navbar_layout) AND empty( $hub2b_theme_config['navbar'][$navbar_layout] ) )
 ...
 * Return navbar menu
*/
if ( !function_exists( 'hub2b_get_wp_nav_menu' ) ) {

	function hub2b_get_wp_nav_menu() {
 ...
 */
if ( !function_exists( 'hub2b_get_sections' ) ) {

	function hub2b_get_sections() {
 ...
 */
if ( !function_exists( 'hub2b_get_pageheader_layout' ) ) {

	function hub2b_get_pageheader_layout() {
 ...
 */
if ( !function_exists( 'hub2b_get_pageheader_class' ) ) {

	function hub2b_get_pageheader_class() {
		
		$hub2b_header_class = array();
		$hub2b_h1 = hub2b_get_the_h1();

		if ( !empty($hub2b_h1) ) {

			$hub2b_header_class[] = ' header-h1 ';
		}
		...
		if ( function_exists('FW') ) {
			...
			if ( $header_fixed == 'fixed' ) {
				...
				$hub2b_header_class[] = ' header-parallax ';
			}
		}

		if ( function_exists( 'bcn_display' ) && !is_front_page() ) {

			$hub2b_header_class[] = ' hasBreadcrumbs';
		}

		$navbar_layout = 'white';
		if ( function_exists( 'FW' ) ) {

			$navbar_layout = hub2b_get_navbar_layout('white');
		}

		$hub2b_header_class[] = ' wrapper-navbar-layout-' . $navbar_layout;



		return implode( ' ', $hub2b_header_class );
	}
}
 ...
 */
if ( !function_exists( 'hub2b_get_pageheader_wrapper' ) ) {

	function hub2b_get_pageheader_wrapper() {
 ...
 */
if ( !function_exists( 'hub2b_check_active_sidebar' ) ) {

	function hub2b_check_active_sidebar() {

		if ( hub2b_is_wc('woocommerce') || hub2b_is_wc('shop') || hub2b_is_wc('product') ) {
 ...
 */
if ( !function_exists( 'hub2b_get_wc_sidebar_pos' ) ) {

	function hub2b_get_wc_sidebar_pos() {

		if ( hub2b_is_wc('product') ) {
		...
		if ( function_exists( 'FW' ) ) {
			...
			if ( hub2b_is_wc('product') ) {
				...
				$hub2b_sidebar = fw_get_db_settings_option( 'shop_post_sidebar' );
			}	
				else {

				$hub2b_sidebar = fw_get_db_settings_option( 'shop_list_sidebar' );
			}
 ...
 */
if ( !function_exists( 'hub2b_custom_css' ) ) {

	function hub2b_custom_css( $css = null ) {
 ...
 */
if ( !function_exists( 'hub2b_find_http' ) ) {

	function hub2b_find_http( $string ) {
 ...
 * 
 */
function hub2b_the_img_srcset( $attachment_id, $sizes_hooks, $sizes_media ) {
...
$hub2b_current_scheme =  apply_filters ('hub2b_current_scheme', array());
