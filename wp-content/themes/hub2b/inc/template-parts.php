 ...
 */
if ( !function_exists( 'hub2b_single_comment' ) ) {

	function hub2b_single_comment( $comment, $args, $depth ) {

		$GLOBALS['comment'] = $comment;
		switch ( $comment->comment_type ) {
			case 'pingback' :
				?>
				<li class="trackback"><?php esc_html_e( 'Trackback:', 'hub2b' ); ?> <?php comment_author_link(); ?><?php edit_comment_link( esc_html__( 'Edit', 'hub2b' ), '<span class="edit-link">', '<span>' ); ?>
				<?php
				break;
			case 'trackback' :
				?>
				<li class="pingback"><?php esc_html_e( 'Pingback:', 'hub2b' ); ?> <?php comment_author_link(); ?><?php edit_comment_link( esc_html__( 'Edit', 'hub2b' ), '<span class="edit-link">', '<span>' ); ?>
				<?php
				break;
			default :
					...
					<article id="comment-<?php comment_ID(); ?>" class="comment-body comment-single">
						...
						<div class="comment-content">
							<div class="comment-info">
								...
								<div class="comment_not_approved"><?php esc_html_e( 'Your comment is awaiting moderation.', 'hub2b' ); ?></div>
								<?php } ?>	                            
							</div>
 ...
 */
if ( !function_exists( 'hub2b_the_before_footer' ) ) {
	
	function hub2b_the_before_footer() {
 ...
 */
if ( !function_exists( 'hub2b_the_subscribe_block' ) ) {
	
	function hub2b_the_subscribe_block() {
 ...
 */
if ( !function_exists( 'hub2b_the_topbar_block' ) ) {

	function hub2b_the_topbar_block( $navbar_layout ) {
 ...
 */
if ( !function_exists( 'hub2b_the_footer_logo_section' ) ) {

	function hub2b_the_footer_logo_section() {
        ...
        $hub2b_logo = fw_get_db_settings_option( 'logo' );
		?>
			...
			<div class="container">
	            <?php
					...
					if ( !empty($hub2b_logo) ) {
						...
						echo '<span class="logo-footer">'.wp_get_attachment_image( $hub2b_logo['attachment_id'], 'full' ).'</span>';
					}

					hub2b_the_social_footer();
	            ?>
			</div>
		</section>
		<?php		
 ...
 */
if ( !function_exists( 'hub2b_the_footer_icons' ) ) {

	function hub2b_the_footer_icons() {
 ...
 * Prints Footer widgets block
 */
if ( !function_exists( 'hub2b_the_footer_widgets' ) ) {

	function hub2b_the_footer_widgets( $layout = null ) {
	    ...
	    if ( $hub2b_footer_cols['num'] > 0 ): ?>
		<section id="ltx-widgets-footer" class="<?php echo esc_attr($footer_class); ?>" >
			<div class="container">
				<div class="row">
	                <?php
	                for ($x = 1; $x <= 4; $x++): ?>
	                    <?php if ( !isset($hub2b_footer_cols['hidden'][ $x ]) && is_active_sidebar( 'footer-' . $x ) ): ?>
						<div class="<?php echo esc_attr( $hub2b_footer_cols['classes'][$x] ).' '.esc_attr( $hub2b_footer_cols['hidden_mobile'][$x] ).' '.esc_attr( $hub2b_footer_cols['hidden_md'][$x] ); ?> clearfix">    
...
if ( !function_exists( 'hub2b_get_the_logo' ) ) {
	...
	function hub2b_get_the_logo( $layout = null ) {
		...
		if ( function_exists( 'FW' ) ) {
			...
			$current_scheme =  apply_filters ('hub2b_current_scheme', array());
	...
	function hub2b_the_logo( $layout = null ) {

		echo hub2b_get_the_logo($layout);
	}
}
 ...
 */
if ( !function_exists( 'hub2b_the_pageloader_overlay' ) ) {

	function hub2b_the_pageloader_overlay() {
 ...
 */
if ( !function_exists( 'hub2b_the_copyrights' ) ) {

	function hub2b_the_copyrights() {

		if ( function_exists( 'FW' ) ) {

			$hub2b_copyrights = fw_get_db_settings_option( 'copyrights' );

			if ( !empty($hub2b_copyrights) ) {

				echo wp_kses_post( $hub2b_copyrights );	
			}
				else {
				
				echo '<p>'. esc_html__( 'Like-themes &copy; All Rights Reserved - 2019', 'hub2b' ) .'</p>';
			}
		}
			else {
 ...
 */
if ( !function_exists( 'hub2b_the_copyrights_section' ) ) {

	function hub2b_the_copyrights_section() {
 ...
 */
if ( !function_exists( 'hub2b_the_go_top' ) ) {

	function hub2b_the_go_top() {
            	...
            	echo '<a href="#" class="ltx-go-top '.esc_attr(implode(' ', $class)).'">';
            		...
            		if ( !empty($go_top_img) ) {
	                	...
	                	if ( empty( $go_top_text ) ) {
	                		...
	                		$go_top_text = esc_html__( 'Go Top', 'hub2b' );
 ...
 * Blog related posts
 */
if ( !function_exists( 'hub2b_related_posts' ) ) {

	function hub2b_related_posts($id) {
		...
		if ( !empty( $tags ) ) {
			...
			if ( $my_query->have_posts() ) {
				...
				set_query_var( 'hub2b_featured_disabled', true );
				...
				echo '<div class="heading has-subheader align-center subcolor-main">';
					echo sprintf(
 					...
 					esc_html__( 'Related ', 'hub2b' ),
 					esc_html__( 'posts', 'hub2b' )
 				);
				echo '</div>';
			...
			wp_reset_postdata();
			set_query_var( 'hub2b_featured_disabled', false );
		}
	}
}
 ...
 */
if ( !function_exists( 'hub2b_author_bio' ) ) {

	function hub2b_author_bio( ) {
		...
		if ( is_single() && isset( $post->post_author ) ) {
			...
			$author_details .= '<p class="author-links">';
				...
				if ( ! empty( $user_website ) ) {
					...
					$author_details .= '<a href="' . esc_url( $user_website ) .'" class="btn btn-main color-hover-white btn-xs" target="_blank" rel="nofollow">'. esc_html__( 'Website', 'hub2b' ) . '</a>';
				}

				$author_details .= '<a href="'. esc_url( $user_posts ) .'" class="btn btn-xs btn-main">'. esc_html__( 'All posts', 'hub2b' ) . '</a>';  
			...
			$content = '<section class="ltx-author-bio">';
				...
				$content .= '<div class="author-info">';
					if ( ! empty( $display_name ) ) {
						...
						$content .= '<span><p class="author-name">'. esc_html__( 'Author', 'hub2b' ) . '</p><h5>'. $display_name . '</h5></span>';
					}				
...
if ( !function_exists( 'hub2b_get_the_post_headline' ) ) {
	...
	function hub2b_get_the_post_headline( $headline = 'inline' ) {
	...
	function hub2b_get_the_post_headline_left() {

		echo '<div class="ltx-post-headline">';

		echo '<div class="ltx-user">'.get_avatar( get_the_author_meta('user_email'), 50 ).'<span class="info">'. esc_html__( 'by', 'hub2b' ) . ' ' .get_the_author_link().'</span></div>';		
...
if ( !function_exists( 'hub2b_the_blog_date' ) ) {
	...
	function hub2b_the_blog_date( $args = array() ) {
...
if ( !function_exists( 'hub2b_get_the_cats_archive' ) ) {

	function hub2b_get_the_cats_archive() {

		if ( in_array( 'category', get_object_taxonomies( get_post_type() ) ) && hub2b_categorized_blog() ) {
...
if ( !function_exists( 'hub2b_the_post_info' ) ) {
	...
	function hub2b_the_post_info( $showText = true, $wrapper = true ) {
 ...
 * 
 */
if ( !function_exists( 'hub2b_the_navbar_icons' ) ) {

	function hub2b_the_navbar_icons( $layout = null, $mobile = false ) {
			...
			foreach ($icons as $item) {
				...
				if ( $item['type']['type_radio'] == 'search') {
					...
					if ( !empty($mobile) ) {
						...
						$close = '<a href="#" id="top-search-ico-close" class="top-search-ico-close " aria-hidden="true">&times;</a>';
					}
					...
					<li class="ltx-fa-icon ltx-nav-search  '.esc_attr($li_class).'">
						<div class="top-search" data-base-href="'. esc_url( home_url( '/' ) ) .'">
							...
							'.$close.'
							<input placeholder="'.esc_attr__( 'Search', 'hub2b' ).'" value="'. esc_attr( get_search_query() ) .'" type="text">
						</div>
					</li>';
				}

				if ( $item['type']['type_radio'] == 'basket' AND hub2b_is_wc('wc_active')) {
					...
					$items .= '
						<li class="ltx-fa-icon ltx-nav-cart '.esc_attr($li_class).'">
							<div class="cart-navbar">
								<a href="'. wc_get_cart_url() .'" class="ltx-cart cart shop_table" title="'. esc_attr__( 'View your shopping cart', 'hub2b' ). '">';
 ...
 */
if ( !function_exists( 'hub2b_the_breadcrumbs' ) ) {

	function hub2b_the_breadcrumbs() {
 ...
 * Tagline in header
 */
if ( !function_exists( 'hub2b_the_tagline_header' ) ) {

	function hub2b_the_tagline_header() {
 ...
 */
if ( !function_exists( 'hub2b_the_social_header' ) ) {

	function hub2b_the_social_header() {

		if ( function_exists( 'FW' ) ) {
			
			$hub2b_social = fw_get_db_settings_option( 'header-social' );
			$hub2b_social_text = fw_get_db_settings_option( 'header-social-text' );

			if ( $hub2b_social == 'enabled' ) {
 ...
 */
if ( !function_exists( 'hub2b_the_social_footer' ) ) {

	function hub2b_the_social_footer() {

		// In this theme we are using same markup as a header
		hub2b_the_social_header();
	}
}		
 ...
 */
if ( !function_exists( 'hub2b_the_navbar_social' ) ) {

	function hub2b_the_navbar_social() {
