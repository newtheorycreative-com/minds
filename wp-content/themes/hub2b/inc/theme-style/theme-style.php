...
if ( !function_exists('hub2b_generate_css') ) {

	function hub2b_generate_css() {
		...
		if ( function_exists( 'FW' ) ) {
			...
			$hub2b_color_schemes = array();
			$hub2b_color_schemes_ = fw_get_db_settings_option( 'items' );

			if ( !empty($hub2b_color_schemes_) ) {
				foreach ($hub2b_color_schemes_ as $v) {

					$hub2b_color_schemes[$v['slug']] = $v;
				}			
			}
		}

		$hub2b_current_scheme =  apply_filters('hub2b_current_scheme', array());	
		if ($hub2b_current_scheme == 'default' OR empty($hub2b_current_scheme)) $hub2b_current_scheme = 1;

		if ( function_exists( 'FW' ) AND !empty($hub2b_current_scheme) ) {
			...
			foreach (array(
				...
				if ( !empty($hub2b_color_schemes[$hub2b_current_scheme][$v]) ) {

					$css[$k] = esc_attr($hub2b_color_schemes[$hub2b_current_scheme][$v]);
					$css_rgb[$k] = sscanf(esc_attr($hub2b_color_schemes[$hub2b_current_scheme][$v]), "#%02x%02x%02x");
				}
			}
		}


		$css['black_darker_color'] = hub2b_adjustBrightness($css['black_color'], -50);
		$css['main_darker_color'] = hub2b_adjustBrightness($css['main_color'], -30);
		$css['main_lighter_color'] = hub2b_adjustBrightness($css['main_color'], 30);

		$css = hub2b_get_google_fonts($css);		
		...
		if ( function_exists( 'FW' ) ) {
			...
			if (! empty( $header_bg ) OR $featured_bg == 'enabled'  ) {
				...
				if ( hub2b_is_wc('product_category') OR hub2b_is_wc('product_tag') ) {
				...
				if ( !empty( $wc_bg['url'] ) AND hub2b_is_wc('shop') ) {
				...
				if ( !empty( $wc_icon['url'] ) AND hub2b_is_wc('shop') ) {
			...
			if ( !empty($pace) AND !empty($pace['image']) AND !empty($pace['image']['loader_img'])) {
				...
				wp_add_inline_style( 'hub2b-theme-style', '.paceloader-image .pace-image { background-image: url(' . esc_attr( $pace['image']['loader_img']['url'] ) . ') !important; } ' );
			}
			...
			if ( !empty($fontello['css']) AND !empty( $fontello['eot']) AND  !empty( $fontello['ttf']) AND  !empty( $fontello['woff']) AND  !empty( $fontello['woff2']) AND  !empty( $fontello['svg']) ) {
				...
				wp_add_inline_style( 'hub2b-theme-style', $css_content );
