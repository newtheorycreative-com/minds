...
if ( !function_exists('hub2b_get_google_fonts') ) {

	function hub2b_get_google_fonts($css = array()) {
		...
		foreach ( array('font_main', 'font_headers', 'font_subheaders') as $type ) {
			...
			if ( !empty($font) ) {
				...
				if ( !empty($font_weights) ) {
					...
					$hub2b_items = explode(',', $font_weights);
					foreach ( $hub2b_items as $item) {
		...
		if ( !empty($google_fonts) ) {
			...
			wp_enqueue_style( 'hub2b-google-fonts', esc_url( add_query_arg( $query_args, '//fonts.googleapis.com/css' ) ), array(), null );
		}
