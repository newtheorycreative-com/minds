...
if ( function_exists( 'FW' ) ) {
	...
	$hub2b_current_scheme_db = fw_get_db_post_option( $wp_query->get_queried_object_id(), 'color-scheme' );
	add_filter ('hub2b_current_scheme', function() {
		global $wp_query;	

		$hub2b_current_scheme_db = fw_get_db_post_option( $wp_query->get_queried_object_id(), 'color-scheme' );
		if ($hub2b_current_scheme_db == 'default') $hub2b_current_scheme_db = 1; 
		return $hub2b_current_scheme_db; 
	} );
...
if ( !hub2b_check_active_sidebar() OR hub2b_is_wc('cart') OR hub2b_is_wc('checkout') ) {
